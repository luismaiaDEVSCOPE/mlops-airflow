...
class PlantCarePromptEngineer:
    ...
    def __init__(self, ollama_url: str = "http://ollama:11434"):
        ...
        self.model_name = "llama2:7b"  # Default small model for education
        self.mlflow_experiment = "plant-care-llmops"
        
    def setup_ollama_model(self):
        """Pull and setup the LLM model in Ollama"""
        logger.info(f"🤖 Setting up Ollama model: {self.model_name}")
...
Structured Response:"""
    ...
    def evaluate_prompt_performance(self, prompt_name: str, prompt: PromptTemplate, 
        ...
        try:
            # Initialize LLM
            llm = Ollama(base_url=self.ollama_url, model=self.model_name)
            ...
            for idx, row in test_data.head(3).iterrows():  # Limit for educational demo
                try:
                    response = chain.run(query=row['customer_query'])
    ...
    def run_llmops_pipeline(self):
        ...
        with mlflow.start_run(run_name=f"llmops_pipeline_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
            ...
            model_ready = self.setup_ollama_model()
            mlflow.log_metric("model_setup_success", 1 if model_ready else 0)
            
            if not model_ready:
                logger.warning("⚠️ Model setup failed, using mock responses for demo")
            ...
            dataset_path = "/tmp/plant_care_dataset.csv"
...
def main():
    ...
    parser.add_argument('--model-name', default='llama2:7b',
                       help='LLM model name')
    ...
    # Initialize pipeline
    pipeline = PlantCarePromptEngineer(ollama_url=args.ollama_url)
